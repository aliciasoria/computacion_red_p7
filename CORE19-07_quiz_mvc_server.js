//haz npm install express@4.16.2
...
const indexView = quizzes =>
    ...
    <table><tr><a href="/quizzes/new" class="button">New Quiz</a></tr></table>
    </body>
    </html>`;
...
const editController = (req, res, next) => {
  const id = Number(req.params.id);
  Quiz.findByPk(id)
  .then((quiz)=>{   res.send(editView(quiz));  }      )
  .catch(next);
...
const updateController = (req, res, next) => {
  const idmia = Number(req.params.id);
  Quiz.update(req.body,{where:{id:idmia}   })
    .then(quiz => res.redirect('/quizzes'))
    .catch(next);
